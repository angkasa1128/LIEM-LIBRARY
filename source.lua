-- Liem UI Library v1.0
-- A modern UI library for Roblox exploits
-- Modified from KyriLib
...
liem.presets = {
    ["liem"] = {
        bg = Color3.fromRGB(10,8,15), container = Color3.fromRGB(16,14,22),
        element = Color3.fromRGB(24,20,32), hover = Color3.fromRGB(34,28,44),
        active = Color3.fromRGB(40,34,54), accent = Color3.fromRGB(255,100,150),
        text = Color3.fromRGB(245,240,255), subtext = Color3.fromRGB(180,170,200),
        border = Color3.fromRGB(40,34,54),
    },
    ["dark"] = {
        bg = Color3.fromRGB(8,8,12), container = Color3.fromRGB(14,14,20),
        element = Color3.fromRGB(20,20,30), hover = Color3.fromRGB(28,28,40),
        active = Color3.fromRGB(34,34,48), accent = Color3.fromRGB(100,180,255),
        text = Color3.fromRGB(240,240,250), subtext = Color3.fromRGB(160,160,185),
        border = Color3.fromRGB(30,30,44),
    },
    ["sunset"] = {
        bg = Color3.fromRGB(15,8,10), container = Color3.fromRGB(22,14,18),
        element = Color3.fromRGB(32,20,26), hover = Color3.fromRGB(44,28,36),
        active = Color3.fromRGB(52,34,44), accent = Color3.fromRGB(255,140,80),
        text = Color3.fromRGB(255,245,240), subtext = Color3.fromRGB(200,180,170),
        border = Color3.fromRGB(46,30,36),
    },
    ["neon"] = {
        bg = Color3.fromRGB(5,5,10), container = Color3.fromRGB(10,10,20),
        element = Color3.fromRGB(16,16,30), hover = Color3.fromRGB(24,24,44),
        active = Color3.fromRGB(30,30,52), accent = Color3.fromRGB(0,255,200),
        text = Color3.fromRGB(220,255,250), subtext = Color3.fromRGB(140,200,190),
        border = Color3.fromRGB(20,30,40),
    },
}

-- Icon pack (using same LucideBlox icons)
liem.icons = {
...
local CONFIG_FOLDER = "LiemConfigs"
...
local function save_config(game_name, config_name, data)
    ...
    writefile(path .. "/" .. config_name .. ".json", liem.svc.http:JSONEncode(encoded))
end

local function load_config(game_name, config_name)
    local path = CONFIG_FOLDER .. "/" .. game_name .. "/" .. config_name .. ".json"
    if isfile(path) then
        local raw = liem.svc.http:JSONDecode(readfile(path))
        local out = {}
...
-- Main UI function
function liem.new(title, options)
    options = options or {}

    if getgenv then
        if getgenv().__liem_inst then
            pcall(function() getgenv().__liem_inst:destroy() end)
            getgenv().__liem_inst = nil
        end
    end

    local localPlayer = liem.svc.plr.LocalPlayer

    local existing = localPlayer.PlayerGui:FindFirstChild("Liem")
    ...
    task.spawn(function()
        -- Try to load logo from URL (you can change this)
        local url = "https://raw.githubusercontent.com/YourUsername/LiemLib/main/liemlogo.png" -- Change this to your logo URL
        local path = "liemlib_logo.png"
        ...
        liem.svc.tw:Create(logo, TweenInfo.new(0.5), {ImageTransparency = 0}):Play()
        task.wait(1.5)
        liem.svc.tw:Create(logo, TweenInfo.new(0.5), {ImageTransparency = 1}):Play()
        task.wait(0.5)
        load_gui:Destroy()
        loaded = true
    end)
    ...
    if options.KeySystem then
        ...
        local fileName = ks.FileName and (ks.FileName .. "_liemkey.txt")
        local th = liem.theme
        ...
        if not savedKeyValid then
            ...
            make("TextLabel", {
                ...
                BackgroundTransparency = 1,
                Text = ks.Title or "Liem Key System",
                TextColor3 = th.text,
                TextSize = 16,
                Font = Enum.Font.GothamBold,
        ...
        local card = make("Frame", {
            ...
            BackgroundColor3 = liem.theme.bg,
            BackgroundTransparency = 1,
            Parent = splashGui
        })
        make("UICorner", { CornerRadius = UDim.new(0, 14), Parent = card })
        make("UIStroke", { Color = liem.theme.border, Thickness = 1, Transparency = 1, Parent = card })

        local accent_bar = make("Frame", {
            Size = UDim2.fromOffset(3, 32),
            Position = UDim2.fromOffset(18, 22),
            BackgroundColor3 = liem.theme.accent,
            BackgroundTransparency = 1,
            Parent = card
        })
        ...
        local splashTitle = make("TextLabel", {
            ...
            BackgroundTransparency = 1,
            Text = ks.Title or title or "Liem UI",
            TextColor3 = liem.theme.text,
            TextTransparency = 1,
            TextSize = 17,
            Font = Enum.Font.GothamBold,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        local splashSub = make("TextLabel", {
            ...
            BackgroundTransparency = 1,
            Text = "by " .. (ks.Creator or "Liem"),
            TextColor3 = liem.theme.subtext,
            TextTransparency = 1,
            TextSize = 13,
            Font = Enum.Font.Gotham,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        liem.svc.tw:Create(card, fadeIn, { BackgroundTransparency = 0 }):Play()
        liem.svc.tw:Create(card:FindFirstChildOfClass("UIStroke"), fadeIn, { Transparency = 0 }):Play()
        liem.svc.tw:Create(accent_bar, fadeIn, { BackgroundTransparency = 0 }):Play()
        liem.svc.tw:Create(splashTitle, fadeIn, { TextTransparency = 0 }):Play()
        liem.svc.tw:Create(splashSub, fadeIn, { TextTransparency = 0 }):Play()
        liem.svc.tw:Create(splashBrand, fadeIn, { TextTransparency = 0 }):Play()

        task.wait(2)

        local fadeOut = TweenInfo.new(0.4)
        liem.svc.tw:Create(card, fadeOut, { BackgroundTransparency = 1 }):Play()
        liem.svc.tw:Create(card:FindFirstChildOfClass("UIStroke"), fadeOut, { Transparency = 1 }):Play()
        liem.svc.tw:Create(accent_bar, fadeOut, { BackgroundTransparency = 1 }):Play()
        liem.svc.tw:Create(splashTitle, fadeOut, { TextTransparency = 1 }):Play()
        liem.svc.tw:Create(splashSub, fadeOut, { TextTransparency = 1 }):Play()
        liem.svc.tw:Create(splashBrand, fadeOut, { TextTransparency = 1 }):Play()
        task.wait(0.5)
        splashGui:Destroy()
    end

    -- Main UI setup (same as before but with Liem branding)
    local w = {}
    local conns = {}

    w.title = title or "Liem"
    w.tabs = {}
    w.active = nil
    w.accents = {}
    w.sounds = {}
    ...
    w.is_mobile = liem.svc.inp.TouchEnabled and not liem.svc.inp.KeyboardEnabled

    local t = {
        bg = (options.Theme and options.Theme.bg) or liem.theme.bg,
        container = (options.Theme and options.Theme.container) or liem.theme.container,
        element = (options.Theme and options.Theme.element) or liem.theme.element,
        hover = (options.Theme and options.Theme.hover) or liem.theme.hover,
        active = (options.Theme and options.Theme.active) or liem.theme.active,
        accent = (options.Theme and options.Theme.accent) or liem.theme.accent,
        text = (options.Theme and options.Theme.text) or liem.theme.text,
        subtext = (options.Theme and options.Theme.subtext) or liem.theme.subtext,
        border = (options.Theme and options.Theme.border) or liem.theme.border
    }
    ...
    if not w.is_mobile then
        ...
        table.insert(conns, liem.svc.inp.InputChanged:Connect(function(inp)
        ...
        table.insert(conns, liem.svc.inp.InputEnded:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                resizing = false
            end
        end))
        ...
        resize_handle.MouseEnter:Connect(function()
            liem.svc.tw:Create(resize_handle, TweenInfo.new(0.2), {ImageColor3 = t.text}):Play()
        end)
        resize_handle.MouseLeave:Connect(function()
            liem.svc.tw:Create(resize_handle, TweenInfo.new(0.2), {ImageColor3 = t.subtext}):Play()
        end)
    end
    ...
    minimize.MouseEnter:Connect(function()
        play("hover")
        liem.svc.tw:Create(minimize, TweenInfo.new(0.2), {ImageColor3 = t.text}):Play()
    end)
    minimize.MouseLeave:Connect(function()
        liem.svc.tw:Create(minimize, TweenInfo.new(0.2), {ImageColor3 = t.subtext}):Play()
    end)
    ...
    minimize.MouseButton1Click:Connect(function()
        ...
        if minimized then
            ...
            liem.svc.tw:Create(main, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
        ...
        liem.svc.tw:Create(minimize, TweenInfo.new(0.25), {
    ...
    local function update_input(input)
        ...
        liem.svc.tw:Create(main, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Position = position
        }):Play()
    end
    ...
    table.insert(conns, liem.svc.run.Heartbeat:Connect(function()
    ...
    if w.is_mobile then
        liem.svc.cas:BindAction("LiemToggle", function(name, state, input)
            if state == Enum.UserInputState.Begin then toggle_gui() end
        end, true, Enum.KeyCode.ButtonR3)
        liem.svc.cas:SetTitle("LiemToggle", "Liem")
        liem.svc.cas:SetPosition("LiemToggle", UDim2.new(1, -70, 1, -70))
    else
        table.insert(conns, liem.svc.inp.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            if input.KeyCode == Enum.KeyCode.RightControl then toggle_gui() end
        end))
    end

    -- Color picker popup (same as before)
    local function make_colorpicker_popup(current, on_change)
        local popup_gui = make("ScreenGui", {
            Name = "LiemColorPicker",
            ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
            ResetOnSpawn = false,
            IgnoreGuiInset = true,
        ...
        local pc_conn = liem.svc.inp.InputChanged:Connect(function(inp)
        ...
        local pe_conn = liem.svc.inp.InputEnded:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                dragging_sv = false
                dragging_hue = false
            end
    ...
    function w:tab(name, icon)
            ...
            if type(icon) == "string" and icon:find("^https://") then
                task.spawn(function()
                    pcall(function()
                        ...
                        local fname = "liem_icons/" .. icon:gsub("[^%w]", "_"):sub(1, 64) .. ".mp4"
                        if not isfolder("liem_icons") then makefolder("liem_icons") end
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            for _, tb in pairs(w.tabs) do
                ...
                liem.svc.tw:Create(tb.btn, ti, {BackgroundColor3 = t.element}):Play()
                liem.svc.tw:Create(tb.txt, ti, {TextColor3 = t.subtext}):Play()
                liem.svc.tw:Create(tb.indicator, ti, {Size = UDim2.new(0, 0, 0, 38)}):Play()
                if tb.icon then
                    liem.svc.tw:Create(tb.icon, ti, {ImageColor3 = t.subtext}):Play()
                end
            end

            page.Visible = true
            liem.svc.tw:Create(btn, ti, {BackgroundColor3 = t.hover}):Play()
            liem.svc.tw:Create(txt, ti, {TextColor3 = t.text}):Play()
            liem.svc.tw:Create(indicator, ti, {Size = UDim2.new(0, 3, 0, 38)}):Play()
            if tab.icon then
                liem.svc.tw:Create(tab.icon, ti, {ImageColor3 = t.text}):Play()
            end
            w.active = tab
        end)
        ...
        btn.MouseLeave:Connect(function()
            if w.active ~= tab then
                liem.svc.tw:Create(btn, ti, {BackgroundColor3 = t.element}):Play()
            end
        end)
        ...
        -- Element functions (same as before but with liem theme)
        ...
        function tab:button(text, callback, icon)
                ...
                if type(icon) == "string" and icon:find("^https://") then
                    task.spawn(function()
                        pcall(function()
                            ...
                            if not res or res.StatusCode ~= 200 then return end
                            if not isfolder("liem_icons") then makefolder("liem_icons") end
                            local fname = "liem_icons/" .. icon:gsub("[^%w]", "_"):sub(1, 64) .. ".png"
                            writefile(fname, res.Body)
                            icon_img.Image = getcustomasset(fname)
                        end)
                    end)
            ...
            click.MouseButton1Click:Connect(function()
                ...
                liem.svc.tw:Create(ripple, TweenInfo.new(0.4), {
            ...
            click.MouseEnter:Connect(function()
                play("hover")
                liem.svc.tw:Create(box, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            click.MouseLeave:Connect(function()
                liem.svc.tw:Create(box, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
            end)
        ...
        function tab:toggle(text, def, callback, flag)
            ...
            local function set_state(new_state, run_callback)
                ...
                liem.svc.tw:Create(tog_bg, ti_tog, {BackgroundColor3 = state and t.accent or t.container}):Play()
                liem.svc.tw:Create(knob, ti_tog, {
                    Position = state and UDim2.new(1, -3, 0.5, 0) or UDim2.new(0, 3, 0.5, 0),
                    AnchorPoint = Vector2.new(state and 1 or 0, 0.5)
                }):Play()
                liem.svc.tw:Create(lbl, ti_tog, {TextColor3 = state and t.text or t.subtext}):Play()
        ...
        function tab:slider(text, min, max, def, callback, flag, step)
            ...
            if w.is_mobile then
                ...
                minus_btn.MouseEnter:Connect(function()
                    liem.svc.tw:Create(minus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                minus_btn.MouseLeave:Connect(function()
                    liem.svc.tw:Create(minus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
                end)
                plus_btn.MouseEnter:Connect(function()
                    liem.svc.tw:Create(plus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                plus_btn.MouseLeave:Connect(function()
                    liem.svc.tw:Create(plus_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
                end)
            end
        ...
        function tab:dropdown(text, options, def, callback, flag)
            ...
            local function close_dropdown()
                ...
                liem.svc.tw:Create(container, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 42)}):Play()
                liem.svc.tw:Create(list_frame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 0)}):Play()
                liem.svc.tw:Create(arrow, TweenInfo.new(0.2), {Rotation = 0}):Play()
                task.wait(0.2)
            ...
            for i, option in ipairs(options) do
                ...
                opt_btn.MouseEnter:Connect(function()
                    liem.svc.tw:Create(opt_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                opt_btn.MouseLeave:Connect(function()
                    liem.svc.tw:Create(opt_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
                end)
            ...
            dropdown_btn.MouseButton1Click:Connect(function()
                ...
                if open then
                    ...
                    liem.svc.tw:Create(container, TweenInfo.new(0.2), {
                        Size = UDim2.new(1, 0, 0, 42 + 34 + max_list + 4)
                    }):Play()
                    liem.svc.tw:Create(list_frame, TweenInfo.new(0.2), {
                        Size = UDim2.new(1, 0, 0, 34 + max_list)
                    }):Play()
                    liem.svc.tw:Create(arrow, TweenInfo.new(0.2), {Rotation = 180}):Play()
                else
                    task.spawn(close_dropdown)
                end
            end)

            table.insert(conns, liem.svc.inp.InputBegan:Connect(function(inp, gpe)
            ...
            dropdown_btn.MouseEnter:Connect(function()
                play("hover")
                liem.svc.tw:Create(dropdown_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            dropdown_btn.MouseLeave:Connect(function()
                liem.svc.tw:Create(dropdown_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
            end)
        ...
        function tab:progressbar(text, max_val)
            ...
            function api:set(val, animate)
                ...
                if animate then
                    liem.svc.tw:Create(fill, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
        ...
        function tab:keybind(text, default, hold_to_interact, callback, flag)
            ...
            keybind_btn.MouseButton1Click:Connect(function()
                ...
                local conn
                conn = liem.svc.inp.InputBegan:Connect(function(input, gpe)
                    if gpe then return end
            ...
            table.insert(conns, liem.svc.inp.InputEnded:Connect(function(input, gpe)
            ...
            keybind_btn.MouseEnter:Connect(function()
                play("hover")
                liem.svc.tw:Create(keybind_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            keybind_btn.MouseLeave:Connect(function()
                liem.svc.tw:Create(keybind_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.container}):Play()
            end)
    ...
    -- Settings tab (same as before but with Liem branding)
    function w:create_settings()
        ...
        local function create_popup(ptitle, message, on_yes)
            local popup_gui = make("ScreenGui", {
                ...
                IgnoreGuiInset = true,
                DisplayOrder = 999999999,
                Parent = liem.svc.plr.LocalPlayer.PlayerGui
            })
            ...
            local function create_btn(label, is_yes)
                ...
                btn.MouseEnter:Connect(function()
                    liem.svc.tw:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                btn.MouseLeave:Connect(function()
                    liem.svc.tw:Create(btn, TweenInfo.new(0.15), {
                        BackgroundColor3 = is_yes and t.accent or t.element
                    }):Play()
                end)
                btn.MouseButton1Click:Connect(function()
        ...
        local function refresh_configs()
            for _, child in ipairs(settings.page:GetChildren()) do
                if child.Name == "LiemConfigRow" then
                    child:Destroy()
                end
            end
            ...
            for _, cfg_name in ipairs(configs) do
                ...
                load_btn.MouseEnter:Connect(function()
                    liem.svc.tw:Create(cfg_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
                end)
                load_btn.MouseLeave:Connect(function()
                    liem.svc.tw:Create(cfg_btn, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
                end)
                delete_btn.MouseEnter:Connect(function()
                    liem.svc.tw:Create(delete_btn, TweenInfo.new(0.15), {ImageColor3 = Color3.fromRGB(255, 100, 100)}):Play()
                end)
                delete_btn.MouseLeave:Connect(function()
                    liem.svc.tw:Create(delete_btn, TweenInfo.new(0.15), {ImageColor3 = t.subtext}):Play()
                end)
        ...
        local preset_names = {}
        for name in pairs(liem.presets) do table.insert(preset_names, name) end
        table.sort(preset_names)
        settings:dropdown("preset", preset_names, "liem", function(name)
            local p = liem.presets[name]
        ...
        for _, pair in ipairs(theme_props) do
            ...
            row.MouseEnter:Connect(function()
                liem.svc.tw:Create(row, TweenInfo.new(0.15), {BackgroundColor3 = t.hover}):Play()
            end)
            row.MouseLeave:Connect(function()
                liem.svc.tw:Create(row, TweenInfo.new(0.15), {BackgroundColor3 = t.element}):Play()
            end)
    ...
    function w:notify(title, text, duration)
        ...
        local notif_gui = w.localPlayer.PlayerGui:FindFirstChild("LiemNotifications")
        if not notif_gui then
            notif_gui = make("ScreenGui", {
                Name = "LiemNotifications",
                ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
                ResetOnSpawn = false,
                IgnoreGuiInset = true,
        ...
        local function dismiss()
            ...
            dismissed = true
            liem.svc.tw:Create(notif, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            liem.svc.tw:Create(bar, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            liem.svc.tw:Create(title_lbl, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            liem.svc.tw:Create(text_lbl, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            liem.svc.tw:Create(prog_fill, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            liem.svc.tw:Create(prog_bg, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            task.wait(0.3)
        ...
        liem.svc.tw:Create(wrapper, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
            Position = UDim2.new(0, 0, 0, 0)
        }):Play()
        liem.svc.tw:Create(prog_fill, TweenInfo.new(duration, Enum.EasingStyle.Linear), {
            Size = UDim2.fromScale(0, 1)
        }):Play()
    ...
    function w:destroy()
        ...
        local ng = w.localPlayer.PlayerGui:FindFirstChild("LiemNotifications")
        if ng then ng:Destroy() end
        if w.is_mobile then
            pcall(function() liem.svc.cas:UnbindAction("LiemToggle") end)
        end
    end

    if getgenv then getgenv().__liem_inst = w end
    return w
end
